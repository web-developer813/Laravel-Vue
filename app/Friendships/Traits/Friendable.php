...
namespace App\Friendships\Traits;

use App\Friendships\Models\Friendship;
use App\Friendships\Models\FriendFriendshipGroups;
use App\Friendships\Status;
...
trait Friendable
{
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function getOnlineFriends($perPage = 0, $groupSlug = '')
    {
        return $this->getOrPaginate($this->getOnlineFriendsQueryBuilder($groupSlug), $perPage);
    }
     ...
     * @return \Illuminate\Database\Eloquent\Collection
     */
    public function getFriendsWhere($perPage = 0, $where = [],$groupSlug = '')
    {
        return $this->getOrPaginate($this->getFriendsSearchQueryBuilder($where,$groupSlug), $perPage);
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    private function getFriendsSearchQueryBuilder($where = [], $groupSlug = '')
    {
        ...
        return $this->where([['id', '!=', $this->getKey()],$where])->whereIn('id', array_merge($recipients, $senders));
    }
     ...
     * @return \Illuminate\Database\Eloquent\Builder
     */
    private function getOnlineFriendsQueryBuilder($groupSlug = '')
    {
        ...
        return $this->where([['id', '!=', $this->getKey()],['status','=',1]])->whereIn('id', array_merge($recipients, $senders));
    }
